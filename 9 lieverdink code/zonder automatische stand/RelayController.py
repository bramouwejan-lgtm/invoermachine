...
class RelayController:
    ...
    def start_stappenmotor(self, richting, steps_per_sec):
        ...
        try:
            ...
            if richting.upper() == 'CW':
                ...
                print(f"WAARSCHUWING: Onbekende richting '{richting}', gebruik 'CW' of 'CCW'")
                return
                
            # 2. Snelheid (Frequentie) en Pulsuitvoer instellen
            if hasattr(self, '_motor_pwm') and self._motor_pwm is not None:
                self._motor_pwm.ChangeFrequency(steps_per_sec) 
                self._motor_pwm.ChangeDutyCycle(self.PULSE_DUTY_CYCLE)
            else:
                print("FOUT: PWM object niet beschikbaar voor stappenmotor!")
        except Exception as e:
            print(f"FOUT bij start_stappenmotor: {e}")
            try:
                self.stop_stappenmotor()
            except:
                pass
    ...
    def stop_stappenmotor(self):
        ...
        try:
            ...
            print(f"FOUT bij stop_stappenmotor: {e}")
    ...
    def lift_omhoog(self, allow_in_automatisch=False, allow_in_handmatig=False):
        """
        Veilige functie om lift omhoog te laten gaan.
        Alleen toegestaan wanneer:
        - Machine in automatisch.py of handmatig.py staat (via parameters)
        - Stappenmotor is op home positie
        - Sensor plank is NIET hoog
        - Sensor lift_boven is NIET hoog
        - Geen noodstop actief
        """
        # Check noodstop eerst (altijd prioriteit)
        if self.btn_noodstop_pushed():
            print("LIFT OMHOOG GEBLOKKEERD: Noodstop is actief!")
            return False
        
        # Check of we in de juiste modus zijn
        if not (allow_in_automatisch or allow_in_handmatig):
            print("LIFT OMHOOG GEBLOKKEERD: Niet in automatisch of handmatig modus!")
            return False
        
        # Check stappenmotor home positie
        if not self.sensor_home_active():
            print("LIFT OMHOOG GEBLOKKEERD: Stappenmotor niet op home positie!")
            return False
        
        # Check sensor plank (mag NIET hoog zijn)
        if self.sensor_plank_active():
            print("LIFT OMHOOG GEBLOKKEERD: Sensor plank is actief (lift moet eerst naar beneden)!")
            return False
        
        # Check sensor lift boven (mag NIET hoog zijn)
        if self.sensor_boven_lift_active():
            print("LIFT OMHOOG GEBLOKKEERD: Lift staat al boven!")
            return False
        
        # Alle checks OK, lift mag omhoog
        self.set_relay(0, True)
        return True

    def lift_omlaag(self, allow_in_reset=False):
        """
        Veilige functie om lift omlaag te laten gaan.
        Alleen toegestaan wanneer:
        - Reset knop is ingedrukt (via parameter)
        - Sensor lift_onder is NIET hoog
        - Geen noodstop actief
        """
        # Check noodstop eerst (altijd prioriteit)
        if self.btn_noodstop_pushed():
            print("LIFT OMLAAG GEBLOKKEERD: Noodstop is actief!")
            return False
        
        # Check of we in reset modus zijn
        if not allow_in_reset:
            print("LIFT OMLAAG GEBLOKKEERD: Niet in reset modus!")
            return False
        
        # Check sensor lift onder (mag NIET hoog zijn)
        if self.sensor_onder_lift_active():
            print("LIFT OMLAAG GEBLOKKEERD: Lift staat al onder!")
            return False
        
        # Alle checks OK, lift mag omlaag
        self.set_relay(1, True)
        return True

    def lift_stop(self):
        """Stop de lift (zowel omhoog als omlaag)."""
        self.set_relay(0, False)
        self.set_relay(1, False)

    def lift_check_noodstop(self):
        """
        Check of noodstop actief is tijdens lift beweging.
        Als ja, stop de lift direct.
        Returns: True als lift is gestopt door noodstop, False anders
        """
        if self.btn_noodstop_pushed():
            print("NOODSTOP: Lift wordt direct gestopt!")
            self.lift_stop()
            return True
        return False
    ...
    def button_handmatig_event(self, channel):
        # Markeer expliciet dat de handmatige knop is ingedrukt
        self.button_handmatig_pressed = True
