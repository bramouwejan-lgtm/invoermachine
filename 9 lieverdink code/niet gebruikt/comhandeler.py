...
#Simpele functies voor seriële communicatie met Meadow Motor Controller
    # Open verbinding
    #serial_conn = serial.Serial('/dev/ttyS0', 9600, timeout=1)
...
def send_command(connection, message, width=None, forward=None):
    """
    ...
    Args:
        ...
        width: Aantal stappen (optioneel)
...
def motor_forward(connection, steps):
    """Stuur motor vooruit."""
    return send_command(connection, "Start", width=steps, forward=True)


def motor_backward(connection, steps):
    """Stuur motor achteruit."""
    return send_command(connection, "Start", width=steps, forward=False)
...
if __name__ == "__main__":
    ...
    conn = serial.Serial('/dev/serial0', 9600, timeout=1)
    time.sleep(4)
